...
#!/bin/bash #bash tells odyssey it is unix
#SBATCH -n 10 #know it is a batch submission; number of cores
#SBATCH -t 1440 # time
#SBATCh -p general # Partition
#SBATCH --contiguous # Cores on same infinibadn switch
#SBATCH --mem=100000 # Total memory
#SBATCH --mail-type=END #E-mail
#SBATCH --mail-user=YOUR_EMAIL

# Next load modules
module load gcc/4.8.2-fasrc01 STAR/2.5c-fasrc02 # load STAR
STAR --genomeDIR $GENOME \ # run STAR; put directory of GENOME; the dash dash is the options
     --readFilesIN /n/stat115/HW3/FAastqFiles/fastq1 $FASTQ2 \ 
     --outFIleNamePrefix ~/start_results \ # out into your home directory
module purge
```

FYI use sbatch filename.sh to run on Odyssey cluster
"sacct" to check the job
"scancel jobID" to cancel the job
output file is going to be "slurm-jobID"

```{r, engine='bash',eval=FALSE}
sbatch STARalignment.sh (example)
```
...
salmon index -t $TRANSCRIPTOME (hg19 genome assembly) -i $INDEX (where to output the index; home directory)
...
```{r, engine='bash',eval=FALSE}
# shebang and cluster specs here
# one dash you can just put the first letter; two dashes you need to be more verbose
module load salmon

salmon quant -i $INDEX \
             -l A \ # A is for automatic. This is giving salmon more specs on the fastq files
             -1 $FASTQ/ENCFF500PDO_sub.fastq\
             -2 $FASTQ/ENCFF708KQE_sub.fastq \
             -o $OUT/ \
             --numBootstraps 100 \
             -p 8 \ # p is for partition, should match the number of cores you are giving it
             --gcBias
...
condition <- c(rep('trmt', 3), rep('control', 3))
names <- c('trmt1', 'trmt2', 'trmt3', 'ctrl1', 'ctrl2', 'ctrl3')
sampleTable = data.frame(sampleName=names, filName=files, condition=condition)
...
```{r}
# Make sure to remove NAs after filtering on p-valuen
...
class myClass(object):
    ...
    def __init__(self, option): # defining function within class; initiating function must be __init__
        self.option = option # (self, option) must follow meaning referring to self function and options
                              # option here can be anything like kitty kat
                              # assign self.option the value of kitty cat

    def myFunction(self, option2="."): # define function within class
...
def readFastq(infile):
  with open(infile, mode="r") as f: # just want reading mode so we do not alter it
  while True:
    name = f.readline().strip() # third entry in each row is name
    if not name:
      break
    seq = f.realine().strip() # second entry in each row
    name2 = f.realine().strip()
    qual = f.realine().strip()
    yield Fastq(name,seq,qual) # yield is similar to return
